...
function decompose(n) {
    ...
    function findSquares(target, max) {
        if (target === 0) return true; // Found valid sequence
        for (let i = Math.min(max, Math.floor(Math.sqrt(target))); i > 0; i--) {
            const square = i * i;
            if (target >= square) {
                result.push(i);
        ...
        return false; // Backtrack failed
    }

    if (findSquares(n * n, n - 1)) {
